...
getStat function can currently return a maximum string size of 8192 characters only.
...
#define  MAX_GET_STAT_LINE_SIZE   8192
...
char  getStats_return_string [MAX_GET_STAT_LINE_SIZE]; /* getStats return string */
...
/* Linked list structure pointers for storing actions and their timings */
...
/* The following function parses an Input jSon pair of action and its timing 
 * and returns a structure pointer of action_struct_rec type.
 */
...
char *getStats ()
{
    ...
    memset (getStats_return_string, 0, sizeof(getStats_return_string));
    ...
    {
        strcpy (getStats_return_string, "\t[\n");
        while (fgets (in_line, MAX_LINE_SIZE, fp) != NULL) {
          strcpy(in_line, remove_trailing_white_space (in_line));
          strcpy(in_line, remove_leading_white_space (in_line));

          sprintf (temp_str, "\t\t{%caction%c:%c%s%c, %cavg%c:%d}\n", 34, 34, 34, 
			 get_field_num_in_line (in_line, 1),34, 34, 34,
                         atoi (get_field_num_in_line (in_line, 4)));

	  if (strlen (getStats_return_string) + strlen (temp_str) > MAX_GET_STAT_LINE_SIZE ) {
              printf("Max getStats string size reached.\n");
	      exit_code=-1;
              return (getStats_return_string);
          }
          strcat (getStats_return_string, temp_str);
        }

        strcat (getStats_return_string, "\t]\n");
        fclose(fp);
    }
    return (getStats_return_string);
}
